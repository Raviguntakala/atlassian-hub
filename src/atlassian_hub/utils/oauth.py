...
# Configure logging
logger = logging.getLogger("atlassian-hub.oauth")
...
KEYRING_SERVICE_NAME = "atlassian-hub-oauth"
...
class OAuthConfig:
    ...
    def _save_tokens_to_file(self, token_data: dict | None = None) -> None:
        ...
        try:
            ...
            token_dir = Path.home() / ".atlassian-hub"
    ...
    def _load_tokens_from_file(client_id: str) -> dict[str, Any]:
        ...
        Returns:
            Dict with the token data or empty dict if no tokens found
        """
        token_path = Path.home() / ".atlassian-hub" / f"oauth-{client_id}.json"
