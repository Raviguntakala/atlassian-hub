"""E2E tests for image retrieval tools against DC instances.

Tests jira_get_issue_images and confluence_get_page_images
via MCP tool calls with real DC Jira/Confluence instances.
"""
...
class TestJiraGetIssueImages:
    """Tests for jira_get_issue_images tool against DC."""
    ...
    async def test_jira_get_issue_images_no_images(
        ...
        # Either "No image" message or summary with downloaded=0
        if text.startswith("{"):
...
class TestConfluenceGetPageImages:
    """Tests for confluence_get_page_images tool against DC."""
    ...
    async def test_confluence_get_page_images_no_images(
        ...
        result = await call_tool(
            mcp_client,
            "confluence_get_page_images",
            {"content_id": dc_instance.test_page_id},
        )
        assert not result.is_error
        assert len(result.content) >= 1
        assert isinstance(result.content[0], TextContent)
...
class TestAcImageConversion:
    ...
    async def test_ac_image_converted_to_markdown(
        ...
        result = await call_tool(
            mcp_client,
            "confluence_get_page",
            {"page_id": dc_image_page},
        )
        assert not result.is_error
        assert isinstance(result.content[0], TextContent)
        page_text = result.content[0].text
        # ac:image → markdown conversion: ![test.png] or ![](url)
