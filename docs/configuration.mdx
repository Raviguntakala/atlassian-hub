...
```json
{
  "mcpServers": {
    "atlassian-hub": {
      ...
      "args": [
        "run", "--rm", "-i",
        "--env-file", "/path/to/your/atlassian-hub.env",
...
See [.env.example](https://github.com/Raviguntakala/atlassian-hub/blob/main/.env.example) for all available options.
...
```bash
uvx atlassian-hub --enabled-tools "confluence_search,jira_get_issue"
```
...
Command-line alternative:
```bash
uvx atlassian-hub --toolsets "default,jira_agile"
```
