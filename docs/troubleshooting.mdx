...
<AccordionGroup>
  ...
  <Accordion title="SSL Certificate Issues">
    atlassian-hub uses the OS native trust store by default (via
...
```bash
# With uvx
npx @modelcontextprotocol/inspector uvx atlassian-hub

# With local development version
npx @modelcontextprotocol/inspector uv --directory /path/to/atlassian-hub run atlassian-hub
```
...
<AccordionGroup>
  ...
  <Accordion title="OAuth Token Expired">
    ...
    **Fix:**
    1. Re-run the OAuth setup: `atlassian-hub --oauth-setup`
...
- Check [GitHub Issues](https://github.com/Raviguntakala/atlassian-hub/issues) for known problems
- Review [SECURITY.md](https://github.com/Raviguntakala/atlassian-hub/blob/main/SECURITY.md) for security-related concerns
