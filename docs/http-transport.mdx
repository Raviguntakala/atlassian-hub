...
```bash
# Using uvx
uvx atlassian-hub --transport streamable-http --stateless --port 9000 -vv
...
STATELESS=true uvx atlassian-hub --transport streamable-http --port 9000
```
...
<Tabs>
  <Tab title="SSE Transport">
    ```bash
    # Using uvx
    uvx atlassian-hub --transport sse --port 9000 -vv
  ...
  <Tab title="Streamable-HTTP">
    ```bash
    # Using uvx
    uvx atlassian-hub --transport streamable-http --port 9000 -vv
...
<Steps>
  <Step title="Run OAuth Setup (if using OAuth)">
    ```bash
    # Using uvx
    uvx atlassian-hub --oauth-setup -v

    # Or using Docker
    docker run --rm -i \
      -p 8080:8080 \
      -v "${HOME}/.atlassian-hub:/home/app/.atlassian-hub" \
      ghcr.io/raviguntakala/atlassian-hub:latest --oauth-setup -v
    ```
  </Step>
  ...
  <Step title="Configure Environment">
    ...
    PUBLIC_BASE_URL=https://mcp.example.com/atlassian-hub
    ```
  </Step>
</Steps>
...
1. Enable minimal OAuth mode:
   ```bash
   # Using uvx
   ATLASSIAN_OAUTH_ENABLE=true uvx atlassian-hub --transport streamable-http --port 9000
